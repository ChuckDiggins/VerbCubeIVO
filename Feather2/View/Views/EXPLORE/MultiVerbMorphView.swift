...
//            ListVerbModelsView(languageViewModel: languageViewModel)
    ...
    func loadVerbList(){
        ...
        verbList = removeVerbsThatAreReflexivesOrHaveResidualPhrases()
        ...
        currentVerbModel = languageViewModel.findModelForThisVerbString(verbWord: verbList[0].getWordAtLanguage(language: currentLanguage))
    }
    ...
    func setPatternStuff(){
        var patternList = languageViewModel.getPatternsForThisModel(verbModel: currentVerbModel)
        stemString = ""
        orthoString = ""
    ...
    func removeVerbsThatAreReflexivesOrHaveResidualPhrases()->[Verb]{
        let vu = VerbUtilities()
        var newVerbList = [Verb]()
        var verbString = ""
        
        for verb in verbList {
            verbString = verb.getWordAtLanguage(language: currentLanguage)
            switch currentLanguage {
            case .Spanish:
                let result = vu.analyzeSpanishWordPhrase(testString: verbString)
                if result.isReflexive || result.residualPhrase.count > 0 { continue }
